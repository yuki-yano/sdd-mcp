...
import type { ToolResult, ValidateGapInput } from '../mcp-tools-types.js'
import { loadTemplate } from '../template-loader.js'
import { renderTemplate } from '../template-renderer.js'

/**
 * Handler for the validate-gap tool.
 * @param input Feature name.
 * @param templateDir Template directory path used for tests.
 * @returns Tool execution result.
 */
export const handleValidateGap = async (input: ValidateGapInput, templateDir?: string): Promise<ToolResult> => {
  ...
  // 2. Load the template.
  const template = await loadTemplate('validate-gap', templateDir)
